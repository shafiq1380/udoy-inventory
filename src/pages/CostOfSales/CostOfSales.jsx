...
import { cstSlsDownloadVoucherReport, cstSlsShowPdfGenerator } from './CostOfSalesPdf';
...
const CostOfSales = () => {
    ...
    const incomeShowPdf = () => {
        setLoading(!loading)
        const pefLink = cstSlsShowPdfGenerator(fstPrdStrDate, fstPrdEndDate, sndPrdStrDate, sndPrdEndDate, trdPrdStrDate, trdPrdEndDate);
        pefLink.then(res => {
            setLoading(false)
            setpdfLink(res)
        })
    ...
    return (
        <div className='page-content'>
            <Container fluid>
                <Breadcrumb title={'Cost Of Sales'} breadcrumbItem={' Cost Of Sales'} />
                <Row>
                    <Col xs="12">
                        <Card>
                            <CardBody>
                                ...
                                {/* period 3 */}
                                <Label for="transactionFrom" className='text-primary' size="lg">Period 3</Label>
                                <Row>
                                    <Col md={4} row>
                                        <FormGroup row>
                                            <Label for="transactionFrom" size="lg">Start Date</Label>
                                            <Col md={10}>
                                                <InputGroup size='lg'>
                                                    <Flatpickr
                                                        ...
                                                        id="date"
                                                        name="date"
                                                        onChange={(selectedDates, dateStr) => setTrdPrdStrDate(dateStr)}
                                                        onClose={(selectedDates, dateStr) => setTrdPrdStrDate(dateStr)}
                                                        onReady={(selectedDates, dateStr, instance) => {
                                                            const inputElement = instance.altInput;
                                                            if (inputElement) {
                                                                inputElement.addEventListener('focus', (e) => e.target.select());
                                            ...
                                            <Col md={10}>
                                                <InputGroup size='lg'>
                                                    <Flatpickr
                                                        ...
                                                        id="date"
                                                        name="date"
                                                        onChange={(selectedDates, dateStr) => setTrdPrdEndDate(dateStr)}
                                                        onClose={(selectedDates, dateStr) => setTrdPrdEndDate(dateStr)}
                                                        onReady={(selectedDates, dateStr, instance) => {
                                                            const inputElement = instance.altInput;
                                                            if (inputElement) {
                                                                inputElement.addEventListener('focus', (e) => e.target.select());
