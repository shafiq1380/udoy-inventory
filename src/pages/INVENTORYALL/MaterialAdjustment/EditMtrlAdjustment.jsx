...
const EditMtrlAdjustment = () => {
    ...
    const fetchTransactionLitem = () => {
        ...
        Post('/api/InvTransaction/GetTransactionByID', data)
            .then((res) => {
                ...
                setAdjustment(adjustList.find((item) => item.value === res?.data?.data[0]?.materialComponent?.trnTypeId === 11 ? 1 : 2))
            })
    };
    ...
    const updateMtrlAdjustment = () => {
        ...
        const data = {
            data: {
                invTransactionHdr: { ...tranHeaderData, updateBy: userID, additionalInfo: addInfo, trnPartyID: adjustment?.value || tranHeaderData.trnPartyID, userCode: userID },
                invTransactionDet: materialComponent.map((item) => {
                    ...
                    return {
                        ...
                        hdrID: 0,
                        lno: 0,
                        trnTypeId: adjustment?.value === 1 ? 10 : 11,
                        trnType: adjustment?.value === 1 ? 'RC' : 'IS',
                        trnCode: "ADJ",
                        itemID: item.itemID,
    ...
    return (
        <div className="page-content">
            <Container fluid>
                <Breadcrumb title="Transaction" breadcrumbItem="Edit Material Adjustment" />
                <CardBody>
                    <Button
                        type="button"
                        color="success"
                ...
                <Card>
                    <CardBody>
                        <Row className='mb-3 '>
                            <Col sm="12" md="6" lg="3">
                                <Label for="transactionDate" size='lg'>Adjustment Date</Label>
                                <InputGroup>
                                    <Flatpickr
                            ...
                            <Col sm="12" md="6" lg="3">
                                <Label for="transactionDate" size='lg'>Adjustment Type</Label>
                                <Select
                                    styles={customStyles}
                                    onChange={(event) => setAdjustment(event)}
                                    options={adjustList.map((item) => {
                                        return {
                            ...
                            <Row style={{ zIndex: -10 }}>
                                <Col className="text-center" >
                                    <button
                                        ...
                                        onClick={updateMtrlAdjustment}
                                    >
                                        Update Meterial Adjustment
                                    </button>
                                </Col>
                            </Row>
                        }
            ...
            <CmnSaveEditResponseModal
                ...
                modalTitle={" Updated Material Return"}
                responseId={responseId}
                error={error}
                nvurl={'/material-adjustment'}
            />
...
export default EditMtrlAdjustment