...
const EditTransactionIssuePage = () => {
    ...
    const fetchTransactionLitem = () => {
        ...
        Post('/api/InvTransaction/GetTransactionByID', data)
            .then((res) => {
                ...
                setConsumption(consumpTionList.find((item) => item.value === res?.data?.data[0]?.invTransactionHdr?.trnCode))
            })
    };
    ...
    const updateReqIssue = () => {
        ...
        const data = {
            data: {
                invTransactionHdr: { ...tranHeaderData, updateBy: userID, additionalInfo: addInfo, trnCode: consumption?.value, userCode: userID },
    ...
    // console.log(tranHeaderData?.requisitionTo)

    // console.log(tranHeaderData)

    return (
        <div className="page-content custom-fontSize">
            <Container fluid>
                <Breadcrumb title="Transaction" breadcrumbItem="Edit Material Issue Purchase" />
                <CardBody>
                    <Button
                        type="button"
                        color="success"
                ...
                <Card>
                    <CardBody>
                        <Row className='mb-3 '>
                            ...
                            <Col sm="12" md="2" lg="2">
                                <Label for="transactionDate" size='lg'>Consumption</Label>
                                <Select
                                    styles={customStyles}
                            ...
                            <Col sm="12" md="2" lg="2" >
                                <Label for="transactionDate" size='lg'>Issue To</Label>
                                <Select
                                    styles={customStyles}
                                    onChange={(event) => setTranHeaderData({ ...tranHeaderData, requisitionTo: event.label })}
                                    ...
                                    onBlur={(e) => {
                                        if (e.target.value) {
                                            setTranHeaderData({ ...tranHeaderData, requisitionTo: e.target.value })
                                        }
                                    }}
                                    placeholder="Select Issue To"
                                    value={{ label: tranHeaderData?.requisitionTo }}
                                />
                            </Col>

                            <Col sm="12" md="2" lg="2">
                                <Label for="issueDate" size='lg'>Issue Date</Label>
                                <InputGroup>
                                    <Flatpickr
                                        className="form-control"
                                        placeholder="dd/mm/yyyy"
                                        ...
                                        id="date"
                                        name="date"
                                        onChange={(e, date) => setTranHeaderData({ ...tranHeaderData, issueDate: date })}
                                        onClose={(e, date) => setTranHeaderData({ ...tranHeaderData, issueDate: date })}
                                        onReady={(selectedDates, dateStr, instance) => {
                                            const inputElement = instance.altInput;
                                            if (inputElement) {
                                                inputElement.addEventListener('focus', (e) => e.target.select());
                                        ...
                                        value={tranHeaderData?.issueDate}
                                    />
                                </InputGroup>
                            </Col>
                        ...
                        <Row style={{ marginTop: '-150px' }}>
                            ...
                            <Col sm="12" md="2" lg="2">
                                <Label for="transactionDate" size='lg'>Requisition Date</Label>
                                <InputGroup>
                                    <Flatpickr
                                        className="form-control"
                                        placeholder="dd/mm/yyyy"
                                        ...
                                        id="date"
                                        name="date"
                                        onChange={(e, date) => setTranHeaderData({ ...tranHeaderData, requisitionDate: date })}
                                        onClose={(e, date) => setTranHeaderData({ ...tranHeaderData, requisitionDate: date })}
                                        onReady={(selectedDates, dateStr, instance) => {
                                            const inputElement = instance.altInput;
                                            if (inputElement) {
                                                inputElement.addEventListener('focus', (e) => e.target.select());
                                        ...
                                        value={tranHeaderData?.requisitionDate}
                                    />
                                </InputGroup>
                            </Col>
                            ...
                            <Col sm="12" md="3" lg="3">
                                ...
                                <textarea
                                    ...
                                    value={tranHeaderData?.requisitionFor}
                                    onChange={(e) => setTranHeaderData({ ...tranHeaderData, requisitionFor: e.target.value })}
                                ></textarea>

                            </Col>
                            {/* <Col sm="12" md="4" lg="4">
                                <Label for="requistion to" size='lg'>Issue To</Label>
                                <textarea
                                    className="form-control"
                                    id="requistion to"
                                    rows="4"
                                    placeholder="Issue to"
                                    value={tranHeaderData?.requisitionTo}
                                    onChange={(e) => setTranHeaderData({ ...tranHeaderData, requisitionTo: e.target.value })}
                                ></textarea>
                                        ...
                                        <table className="table table-striped table-bordered table-sm" width="100%">
                                            <thead>
                                                <tr>
                                                    ...
                                                    <th className="col-2">Current Stock</th>
                                                                ...
                                                                <td>
                                                                    <Row>
                                                                        <Col md={11}>
                                                                            ...
                                                                            />
                                                                            <p className='m-0'>{selectedTranValues?.[index]?.label ||
                            ...
                            <Row style={{ zIndex: -10 }}>
                                <Col className="text-center" >
                                    <button
                                        ...
                                        onClick={updateReqIssue}
                                    >
                                        Update Requisition & Issue
                                    </button>
                                </Col>
                            </Row>
                        }
            ...
            <CmnSaveEditResponseModal
                ...
                modalTitle={"Updated Purchase Requisition And Issue"}
...
export default EditTransactionIssuePage