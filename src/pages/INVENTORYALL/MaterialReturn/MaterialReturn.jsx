...
const MaterialReturn = () => {
    ...
    const [returenIssue, setreturenIssue] = useState({ value: "RET", label: 'Return From Issue', })
    ...
    const fetchTrnList = () => {
        Post('/api/InvTransaction/GetTransactionTypeList')
            .then((res) => {
                ...
                const data = res?.data?.data.find(item => item.trnCode === returenIssue?.value)?.trnAddValFormat
    ...
    // console.log(returenIssue?.value)
    ...
    const saveMaterialReturn = () => {
            ...
            data: {
                ...
                invTransactionDet: materialComponent.map((item) => {
                    ...
                    return {
                        hdrID: 0,
                        lno: 0,
                        trnTypeId: returenIssue?.value === 'RET' ? 8 : 9,
                        trnType: returenIssue?.value === 'RET' ? 'RC' : 'IS',
                        trnCode: returenIssue?.value,
                        itemID: item.itemID,
                        ...
                        itemSerial: item.isSerial ? item.itemSerial : "",
                        currentStock: 0
                    }
                })
            }
        ...
        if (!returenIssue) {
            toast.error("Please select Return Type", toastOptions);
            return false;
        }
