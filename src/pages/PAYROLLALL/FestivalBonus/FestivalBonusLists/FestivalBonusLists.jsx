...
const FestivalBonusLists = () => {

    const navigate = useNavigate();
    const dispatch = useDispatch();

    useEffect(() => {
        dispatch(fetchAllowanceListsByTypeRequest(2));
    }, [dispatch]);
                ...
                Cell: (festivalInfo) => {
                    return (
                        <Button
                            ...
                            onClick={() => viewFestivalBonus(festivalInfo.row.original)}
                        >
                            View Details
                        </Button>
                    );
                                ...
                                festivalInfo.row.original.allowStatus === 1 ?
                                    <>
                                        <FaEdit
                                            ...
                                            onClick={() => editFestivalBonus(festivalInfo.row.original)}
                                        />
    ...
    const createFestivalBonus = () => {
        navigate('/create-festival-bonus')
    };

    const editFestivalBonus = (festivalInfo) => {
        navigate('/edit-festival-bonus', { state: festivalInfo })
    };

    const viewFestivalBonus = (festivalInfo) => {
        navigate('/details-festival-bonus', { state: festivalInfo })
    };
    ...
    return (
        <div className='page-content'>
            <Container fluid>
                <Breadcrumbs title="Transaction" breadcrumbItem="Festival Bonus" />
                                ...
                                <Card>
                                    <CardBody>
                                        <TableContainer
                                            ...
                                            onClickBtn={createFestivalBonus}
                                        />
                                    </CardBody>
                                </Card>
                            </Col>
...
export default FestivalBonusLists