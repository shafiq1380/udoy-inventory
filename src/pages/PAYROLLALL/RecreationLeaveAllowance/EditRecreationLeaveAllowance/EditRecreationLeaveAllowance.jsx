...
const EditRecreationLeaveAllowance = () => {
    ...
    useEffect(() => {
        const data = allowanceDataUpdateById && allowanceDataUpdateById.map((item) => {
            ...
            const newBasic = item.totAdition;
            return {
                ...item,
                basic,
                basicSal,
                ...
                netPayable: newBasic > 0 ? newBasic - (item.revStamp || 0) : 0,
            }
        });
    ...
    const handleInputChange = (e, index) => {
        ...
        if (name === 'newBasic') {
            item.newBasic = parseValue(value);
            ...
            if (item.newBasic === 0 || item.newBasic === '') {
                item.revStamp = '';
                item.netPayable = '';
            } else {
                item.revStamp = item.revStamp || 0;
                item.netPayable = item.newBasic > 0 ? roundToTwoDecimals(item.newBasic - (item.revStamp || 0)) : 0;
            }
        } else if (name === 'revStamp' && item.newBasic > 0 || item.basic > 0) {
            ...
            item.netPayable = item.newBasic > 0 ? roundToTwoDecimals(item.newBasic - item.revStamp) : item.basic > 0 ? roundToTwoDecimals(item.basic - item.revStamp) : 0;
        }
        ...
        const data = {
            data: {
                ...
                detailData: filteredData.map((item) => ({
                    ...
                    otRate: 0,
                    otHour: 0,
                    totalAddition: item.newBasic ? item.newBasic : item.basic,
                    revStamp: item.revStamp,
    ...
    const editRecreationAllowance = allowanceData && allowanceData.length > 0 ? false : true;

    return (
        <div className="page-content">
            <Container fluid>
                <Breadcrumbs title="Transaction" breadcrumbItem="Recreation Leave Allowance Edit" />
                <Button
                    type="button"
                    color="success"
                ...
                <Card className="mt-2">
                    <CardBody>
                        <CardHeader>
                            <h3 className="text-center text-uppercase">Recreation Leave Allowance Edit Screen</h3>
                        </CardHeader>
                    </CardBody>
                </Card>

                {
                    editRecreationAllowance ? <div className="text-center"><CustomSpinner /></div>
                        :
                        <>
                            <Card className="mt-3">
                                <CardBody>
                                                    ...
                                                    <tr key={index}>
                                                        ...
                                                        <td className="text-end">
                                                            <Input
                                                                ...
                                                                placeholder="Basic"
                                                                id="newBasic"
                                                                value={item.newBasic !== null ? item.newBasic : item.basic}
...
export default EditRecreationLeaveAllowance;